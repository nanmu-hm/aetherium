...
def _goal_supports_action(character: CharacterState, action_type: str) -> bool:
    ...
    return bool(goal and action_type in goal.preferred_actions)
...
def generate_action_pool(state: WorldState, character_id: str) -> list[ActionCandidate]:
    ...
    freedom_pressure = character.human_condition.effective_desire("freedom", character.location)
    ...
    if pressure == 0.0:
        ...
        pool.append(ActionCandidate(
            ...
            actor_id=character.id,
            action_type="rest",
            motivation="rest while immediate pressures are low",
            preconditions=["current human-condition pressure is low"],
